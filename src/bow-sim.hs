...
nullWordSim = WordSim (-1) 0 0 0 0 0 [] [] []
...
main = do
  ...
  if not $ gotArg args 0 then do
    ...
    forM_ (zip [1..] ps) $ \(i,(w1,w2)) -> do
      c <- fromMaybe nullWordSim <$> wordSim w1 w2
      liftIO . putStrLn $ 
        printf "(%04d) w1 = %s, w2 = %s, cos(v1,v2) = %.4f, norm2(v1) = %.2f, norm2(v2) = %.2f, norm2(v1*v2) = %.2f, dot(v1,v2) = %.2f, dim_shared(v1,v2) = %d\n\n"
          (i::Int) w1 w2 (cosine c) (norm1 c) (norm2 c) (norm12 c) (dotProd c) (dimShared c) ++ 
